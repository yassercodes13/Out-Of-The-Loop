...
def textify(text: TextRef | list[TextRef], lang = "en", use_b : bool = False):
  if isinstance(text, TextRef):
    ...
    resolved = {
      k: textify(v, lang) if isinstance(v, (tuple, list)) else v
...
def build_buttons(raw_markup: list[list[Button]], chat_id: str, lang = "en"):
  ...
  for row in raw_markup:
    raw_buttons = []
    for btn in row:
      assert isinstance(btn, Button), f"Expected Button, got {type(btn)}: {btn!r}"
      text_ref = btn.text
...
async def edit_message(session: Session, text: TextRef | list[TextRef], buttons: list[list[Button]] = None, parse_mode: str = None):
  ...
  raw_markup = buttons
  buttons = build_buttons(raw_markup, session.chat_id, lang) if buttons else None
...
async def broadcast_message(game: Game, mode: str, text: TextRef | list[TextRef], buttons: list[list[Button]] = None, parse_mode: str = None, exclude_chat_ids: list[int] = None, only_with_substate: AnySubstate = None):
