...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
  ...
  if not query.data or not query.data.startswith(("s:", "g:")):
    return

  session, game = get_session_game(update)

  is_starting_new_game = query.data == "s:setup_game"

  if not game and not is_starting_new_game:
    await query.answer(text = "You have no running game.")
    return
  
  if not is_active(update) and not is_starting_new_game:
...
callback_handler = CallbackQueryHandler(handle_callback, pattern = r"^(g:|s:)")
